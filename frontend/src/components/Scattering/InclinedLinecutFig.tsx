...
const InclinedLinecutFig: React.FC<InclinedLinecutFigProps> = ({
    ...
    const { curves, legendEntries, xDomain, yDomain } = useMemo(() => {
        ...
        linecuts
            ...
            .forEach((linecut) => {
                ...
                entries.push({ id: `left-${linecut.id}`, label: leftLabel, color: linecut.leftColor });
    ...
    if (curves.length === 0) {
        return (
            <div className="w-full h-full flex items-center justify-center">
                <p className="text-lg text-gray-500">No inclined linecut data available</p>
            </div>
        );
    }
        ...
        <div className="w-full h-full flex flex-col" data-linecut-fig="true">
            ...
            <div className="w-full flex-1 h-0 flex flex-col">
                ...
                >
                    ...
                    <TooltipMesh
                        guides="both"
                        renderTooltip={createTooltipRenderer(curves, { xLabel: 'qᵣ', xUnit: units })}
                    />
                </VisCanvas>
            </div>
            {/* Legend */}
